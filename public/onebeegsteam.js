// OneBeigeSteam button injection (standalone plugin)
...
(function () {
  ...
  gamepadCSS.textContent = `
        ...
        .onebeegsteam-button.active-focus,
        .onebeegsteam-restart-button.active-focus {
  ...
  // ALL OneBeigeSteam overlays that should block Steam navigation
  const OVERLAY_SELECTORS = [
    ".onebeegsteam-overlay",
    ".onebeegsteam-settings-overlay",
    ".onebeegsteam-alert-overlay",
    ".onebeegsteam-confirm-overlay",
    ".onebeegsteam-loadedapps-overlay",
  ];
  ...
  function scanFocusableElements() {
    ...
    // Only scan if there's a OneBeigeSteam overlay active
    const activeOverlay = document.querySelector(OVERLAY_SELECTOR_STRING);

    if (!activeOverlay) {
      console.log("[Gamepad] No OneBeigeSteam overlay active, skipping scan");
  ...
  function pollGamepad() {
    ...
    // Check if there's an active OneBeigeSteam overlay
...
(function () {
  ...
  // re-injects this whole script whenever window.__OneBeigeSteamReady isn't true yet — which can happen
  // while THIS injection's own async setup is still in flight (see the retry loop in
  // addOneBeigeSteamButton() and where __OneBeigeSteamReady actually gets set, near the end of this IIFE). Without
  ...
  if (window.__OneBeigeSteamInjected) return;
  window.__OneBeigeSteamInjected = true;
  ...
  // ============================================================
  // OneBeigeSteam GUI backend shim
  // ------------------------------------------------------------
  // The old Lua/Millennium backend is gone. The OneBeigeSteam GUI desktop app is now
  ...
  const Millennium = (function () {
    ...
    const call = (method, args) =>
      window.Millennium.callServerMethod("onebeegsteam", method, args || {}).then(
        (res) => (typeof res === "string" ? JSON.parse(res) : res),
      );

    const map = {
      // ── Real endpoints (OneBeigeSteam GUI HTTP server) ──
      HasOneBeigeSteamForApp: (a) => call("HasOneBeigeSteamForApp", { appid: aid(a) }),
      DeleteOneBeigeSteamForApp: (a) => call("DeleteOneBeigeSteamForApp", { appid: aid(a) }),
      CancelAddViaOneBeigeSteam: (a) => call("CancelAddViaOneBeigeSteam", { appid: aid(a) }),
  ...
  function backendLog(message) {
    try {
      if (
        ...
        Millennium.callServerMethod("onebeegsteam", "Logger.log", {
          message: String(message),
        });
      }
    } catch (err) {
      if (typeof console !== "undefined" && console.warn) {
        console.warn("[OneBeigeSteam] backendLog failed", err);
      }
    }
  }
  ...
  function startOneBeigeSteamAdd(appid, anchor) {
    ...
    window.Millennium.callServerMethod("onebeegsteam", "StartOneBeigeSteamAdd", {
      appid,
    ...
    const q = function (sel) {
      const o = document.querySelector(".onebeegsteam-overlay");
      return o ? o.querySelector(sel) : null;
    };

    const renderSources = function (sources, clickable) {
      const list = q(".onebeegsteam-api-list");
      if (!list) return;
      ...
      sources.forEach(function (s) {
        ...
        item.className = "onebeegsteam-api-item";
        ...
        if (clickable && s.canDownload && !s.downloading) {
          ...
          item.onclick = function () {
            ...
            picking = true;
            renderKey = "";
            const st = q(".onebeegsteam-status");
            if (st) st.textContent = lt("Starting download…");
            window.Millennium.callServerMethod("onebeegsteam", "PickOneBeigeSteamAddSource", {
              appid,
              source: s.name,
            }).catch(function () {});
          };
    ...
    const timer = setInterval(function () {
      ...
      window.Millennium.callServerMethod("onebeegsteam", "GetOneBeigeSteamAddStatus", { appid })
        .then(function (st) {
          if (typeof st === "string") st = JSON.parse(st);
          const overlay = document.querySelector(".onebeegsteam-overlay");
          if (!overlay) {
          ...
          const statusEl = q(".onebeegsteam-status");
          const titleEl = q(".onebeegsteam-title");
          const wrap = q(".onebeegsteam-progress-wrap");
          const bar = q(".onebeegsteam-progress-bar");
          const percent = q(".onebeegsteam-percent");
          ...
          if (st.installed || st.installStatus) {
            ...
            const hide = q(".onebeegsteam-hide-btn");
            if (hide) hide.innerHTML = "<span>" + lt("Close") + "</span>";
            // Game is added → remove the store-page "Add via OneBeigeSteam" button.
            if (anchor && anchor.parentElement)
              anchor.parentElement.removeChild(anchor);
            window.__OneBeigeSteamButtonInserted = false;
  ...
  backendLog("OneBeigeSteam script loaded");
  backendLog(
    "Mode Detection: " +
  ...
  const runState = {
    inProgress: false,
    appid: null,
    pollTimer: null, // id of the active GetOneBeigeSteamAddStatus setInterval, so cancel/close can stop it
  };
  ...
  function fetchGamesDatabase() {
    ...
    return Millennium.callServerMethod("onebeegsteam", "GetGamesDatabase", {
      contentScriptQuery: "",
    })
      .then(function (res) {
      ...
      .catch(function (err) {
        console.warn("[OneBeigeSteam] Failed to fetch games database", err);
        return {};
      });
  }
  ...
  function fetchFixes(appid) {
    ...
    return Millennium.callServerMethod("onebeegsteam", "CheckForFixes", {
      appid: appid,
      contentScriptQuery: "",
    })
      .then(function (res) {
      ...
      .catch(function (err) {
        console.warn("[OneBeigeSteam] Failed to fetch fixes", err);
        return null;
      });
  }
  ...
  function applyTranslationBundle(bundle) {
    ...
    const stored = window.__OneBeigeSteamI18n || {};
    if (bundle.language) {
    ...
    window.__OneBeigeSteamI18n = stored;
  }
  ...
  function loadThemesFromBackend() {
    ...
    return Millennium.callServerMethod("onebeegsteam", "GetThemes", {
      contentScriptQuery: "",
    })
      .then(function (res) {
        try {
  ...
  function getCurrentThemeKey() {
    try {
      const settings = window.__OneBeigeSteamSettings || {};
  ...
  function generateThemeStyles(theme) {
    return `
            ...
            .onebeegsteam-settings-overlay select,
            .onebeegsteam-overlay select,
            .onebeegsteam-loadedapps-overlay select {
            ...
            .onebeegsteam-settings-overlay select option,
            .onebeegsteam-overlay select option,
            .onebeegsteam-loadedapps-overlay select option {
                background-color: ${theme.bgPrimary} !important;
                color: ${theme.text} !important;
            }
            .onebeegsteam-settings-overlay select option:checked,
            .onebeegsteam-overlay select option:checked,
            .onebeegsteam-loadedapps-overlay select option:checked {
                background: ${theme.accent} !important;
                color: ${theme.text} !important;
            }
            .onebeegsteam-settings-overlay select:hover,
            .onebeegsteam-overlay select:hover,
            .onebeegsteam-loadedapps-overlay select:hover {
                border-color: ${theme.borderHover} !important;
            }
            .onebeegsteam-settings-overlay select:focus,
            .onebeegsteam-overlay select:focus,
            .onebeegsteam-loadedapps-overlay select:focus {
            ...
            .onebeegsteam-btn:hover:not([data-disabled="1"]) {
                background: ${theme.bgHover};
                transform: translateY(-2px);
                box-shadow: 0 6px 20px ${theme.shadowHover};
                border-color: ${theme.borderHover};
            }
            .onebeegsteam-btn.primary {
            ...
            .onebeegsteam-btn.primary:hover:not([data-disabled="1"]) {
            ...
            .onebeegsteam-toggle-container {
                display: flex;
                align-items: center;
                justify-content: space-between;
                width: 100%;
            }
            .onebeegsteam-toggle-label-wrap {
                display: flex;
            ...
            input:checked + .onebeegsteam-slider {
                background-color: #1a9fff;
                border-color: #1a9fff;
            }
            input:checked + .onebeegsteam-slider:before {
                transform: translateX(24px);
            }
            .onebeegsteam-slider:hover {
                border-color: #1a9fff;
            }
            ...
            /* Store header button - OneBeigeSteam themed icon button */
            button.onebeegsteam-header-button {
                display: inline-flex;
                align-items: center;
            ...
            button.onebeegsteam-header-button:hover {
                background: ${theme.bgHover};
                transform: translateY(-1px);
                box-shadow: 0 4px 12px ${theme.shadowHover};
                border-color: ${theme.borderHover};
            }
            button.onebeegsteam-header-button:focus-visible {
                outline: 2px solid ${theme.accent};
                outline-offset: 2px;
            }
            button.onebeegsteam-header-button img,
            button.onebeegsteam-header-button svg {
                height: 16px;
                width: 16px;
            }
        `;
  ...
  function ensureThemeStylesheet(themeKey) {
    const id = "onebeegsteam-theme-css";
    ...
    try {
      ...
      backendLog("OneBeigeSteam: Theme CSS injection failed: " + err);
    }
  }

  function ensureOneBeigeSteamStyles() {
    const styleEl = document.getElementById("onebeegsteam-styles");
    ...
    if (styleEl) {
      ...
      try {
        ...
        style.id = "onebeegsteam-styles";
        style.textContent = styles;
        document.head.appendChild(style);
      } catch (err) {
        backendLog("OneBeigeSteam: Styles injection failed: " + err);
      }
    }
  }

  function ensureFontAwesome() {
    if (document.getElementById("onebeegsteam-fontawesome")) return;
    try {
      const link = document.createElement("link");
      link.id = "onebeegsteam-fontawesome";
      link.rel = "stylesheet";
      link.href =
      ...
      backendLog("OneBeigeSteam: Font Awesome injection failed: " + err);
    }
  }

  function showSettingsPopup() {
    if (
      document.querySelector(".onebeegsteam-settings-overlay") ||
      settingsMenuPending
    )
      return;
    ...
    ensureTranslationsLoaded(false)
      ...
      .finally(function () {
        ...
        if (document.querySelector(".onebeegsteam-settings-overlay")) return;

        try {
          const d = document.querySelector(".onebeegsteam-overlay");
          if (d) d.remove();
        } catch (_) {}
        ensureOneBeigeSteamStyles();
        ensureFontAwesome();
        ...
        overlay.className = "onebeegsteam-settings-overlay";
        ...
        titleIcon.alt = "OneBeigeSteam";
        try {
          Millennium.callServerMethod("onebeegsteam", "GetIconDataUrl", {
            contentScriptQuery: "",
          }).then(function (res) {
            try {
              ...
              titleIcon.src =
                p && p.success && p.dataUrl
                  ? p.dataUrl
                  : "OneBeigeSteam/onebeegsteam-icon.png";
            } catch (_) {
              titleIcon.src = "OneBeigeSteam/onebeegsteam-icon.png";
            }
          });
        } catch (_) {
          titleIcon.src = "OneBeigeSteam/onebeegsteam-icon.png";
        ...
        titleText.textContent = t("menu.title", "OneBeigeSteam · Menu");
        ...
        removeBtn.innerHTML =
          ...
          t("menu.removeOneBeigeSteam", "Remove via OneBeigeSteam") +
          "</span>";
        ...
        if (checkBtn) {
          checkBtn.addEventListener("click", function (e) {
            ...
            try {
              Millennium.callServerMethod("onebeegsteam", "CheckForUpdatesNow", {
                contentScriptQuery: "",
              }).then(function (res) {
                try {
                  const payload =
                  ...
                  ShowOneBeigeSteamAlert("OneBeigeSteam", msg);
                } catch (_) {}
              });
            } catch (_) {}
          });
        ...
        if (discordBtn) {
          discordBtn.addEventListener("click", function (e) {
            ...
            const url = "https://discord.gg/onebeegsteam";
            try {
              Millennium.callServerMethod("onebeegsteam", "OpenExternalUrl", {
                url,
                contentScriptQuery: "",
              });
            } catch (_) {}
        ...
        if (settingsManagerBtn) {
          // Settings are managed entirely in the OneBeigeSteam app — open its Settings
          ...
          settingsManagerBtn.addEventListener("click", function (e) {
            ...
            window.Millennium.callServerMethod("onebeegsteam", "OpenSettings", {}).catch(
              function () {},
            );
          });
        }
        ...
        if (fixesMenuBtn) {
          // Fixes are managed entirely in the OneBeigeSteam app — open its Fixes page
          ...
          fixesMenuBtn.addEventListener("click", function (e) {
            ...
            try {
              ...
              const appid = match
                ? parseInt(match[1], 10)
                : window.__OneBeigeSteamCurrentAppId || NaN;
              try {
                overlay.remove();
              } catch (_) {}
              if (isNaN(appid)) {
                ...
                ShowOneBeigeSteamAlert("OneBeigeSteam", errText);
                return;
              }
              window.Millennium.callServerMethod("onebeegsteam", "OpenFix", { appid }).catch(
                function () {},
              );
            } catch (err) {
              backendLog("OneBeigeSteam: Fixes Menu button error: " + err);
            }
          });
        }
        ...
        try {
          ...
          if (
            ...
            Millennium.callServerMethod("onebeegsteam", "HasOneBeigeSteamForApp", {
              appid,
              contentScriptQuery: "",
            }).then(function (res) {
              try {
                ...
                if (exists) {
                    ...
                    try {
                      Millennium.callServerMethod(
                        ...
                        .then(function () {
                          try {
                            window.__OneBeigeSteamButtonInserted = false;
                            window.__OneBeigeSteamGameAdded = false;
                            window.__OneBeigeSteamPresenceCheckInFlight = false;
                            window.__OneBeigeSteamPresenceCheckAppId = undefined;
                            addOneBeigeSteamButton(true);
                            const successText = t(
                              "menu.remove.success",
                              "OneBeigeSteam removed for this app.",
                            );
                            ShowOneBeigeSteamAlert("OneBeigeSteam", successText);
                          } catch (err) {
                            backendLog(
                              "OneBeigeSteam: post-delete cleanup failed: " + err,
                            );
                          }
                        })
                        .catch(function (err) {
                          const failureText = t(
                            "menu.remove.failure",
                            "Failed to remove OneBeigeSteam.",
                          );
                          const errMsg =
                            err && err.message ? err.message : failureText;
                          ShowOneBeigeSteamAlert("OneBeigeSteam", errMsg);
                        });
                    } catch (err) {
                      backendLog("OneBeigeSteam: doDelete failed: " + err);
                    }
                  };
                  ...
                  removeBtn.onclick = function (e) {
                    ...
                    const confirmMessage = t(
                      "menu.remove.confirm",
                      "Remove via OneBeigeSteam for this game?",
                    );
                    showOneBeigeSteamConfirm(
                      "OneBeigeSteam",
                      confirmMessage,
                      function () {
                        doDelete();
  ...
  function ensureTranslationsLoaded(forceRefresh, preferredLanguage) {
    try {
      if (
        !forceRefresh &&
        window.__OneBeigeSteamI18n &&
        window.__OneBeigeSteamI18n.ready
      ) {
        return Promise.resolve(window.__OneBeigeSteamI18n);
      }
      if (
        typeof Millennium === "undefined" ||
        typeof Millennium.callServerMethod !== "function"
      ) {
        window.__OneBeigeSteamI18n = window.__OneBeigeSteamI18n || {
          language: "en",
          locales: [],
          strings: {},
          ready: false,
      ...
      const settingsVals =
        ((window.__OneBeigeSteamSettings || {}).values || {}).general || {};
      const useSteamLang =
      ...
      if (!targetLanguage) {
        ...
        targetLanguage = useSteamLang
          ? steamLang
          : (window.__OneBeigeSteamI18n && window.__OneBeigeSteamI18n.language) || "en";
      }
      return Millennium.callServerMethod("onebeegsteam", "GetTranslations", {
        ...
        .then(function (res) {
          ...
          return window.__OneBeigeSteamI18n;
        })
        .catch(function (err) {
          backendLog("OneBeigeSteam: translation load failed: " + err);
          window.__OneBeigeSteamI18n = window.__OneBeigeSteamI18n || {
            language: "en",
            locales: [],
            strings: {},
      ...
      backendLog("OneBeigeSteam: ensureTranslationsLoaded error: " + err);
      window.__OneBeigeSteamI18n = window.__OneBeigeSteamI18n || {
        language: "en",
        locales: [],
        strings: {},
  ...
  function translateText(key, fallback) {
    ...
    try {
      const store = window.__OneBeigeSteamI18n;
      if (
        store &&
        store.strings &&
  ...
  function askRestartConfirmation() {
    showOneBeigeSteamConfirm(
      ...
      function () {
        try {
          Millennium.callServerMethod("onebeegsteam", "RestartSteam", {
            contentScriptQuery: "",
          });
  ...
  function showTestPopup() {
    // Avoid duplicates
    if (document.querySelector(".onebeegsteam-overlay")) return;
    // Close settings popup if open so modals don't overlap
    try {
      const s = document.querySelector(".onebeegsteam-settings-overlay");
      if (s) s.remove();
    } catch (_) {}
    ...
    ensureFontAwesome();
    const overlay = document.createElement("div");
    overlay.className = "onebeegsteam-overlay";
    overlay.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(12px);z-index:99999;display:flex;align-items:center;justify-content:center;";
    ...
    title.className = "onebeegsteam-title";
    ...
    apiListContainer.className = "onebeegsteam-api-list";
    apiListContainer.style.cssText = "margin-bottom:16px;";

    // NOTE: the source list is populated/updated by startOneBeigeSteamAdd() from the
    ...
    body.className = "onebeegsteam-status";
    body.innerHTML =
    ...
    progressWrap.className = "onebeegsteam-progress-wrap";
    ...
    progressBar.className = "onebeegsteam-progress-bar";
    progressWrap.appendChild(progressBar);

    const progressInfo = document.createElement("div");
    progressInfo.style.cssText = `display:none;margin-top:8px;font-size:12px;color:${colors.textSecondary};`;
    progressInfo.className = "onebeegsteam-progress-info";

    const percent = document.createElement("span");
    percent.className = "onebeegsteam-percent";
    ...
    // .onebeegsteam-hide-btn so the poll's existing selector still finds it.
    const hideBtn = document.createElement("a");
    hideBtn.className = "onebeegsteam-btn onebeegsteam-hide-btn";
    ...
    function cleanup() {
      // The status poll (setInterval in startOneBeigeSteamAdd) lives in another scope — this is the only place a
    ...
    function cancelOperation() {
      try {
        ...
        if (
          ...
          Millennium.callServerMethod("onebeegsteam", "CancelAddViaOneBeigeSteam", {
            appid,
            contentScriptQuery: "",
          });
        }
  ...
  function fetchSettingsConfig(forceRefresh) {
    try {
      if (
        !forceRefresh &&
        window.__OneBeigeSteamSettings &&
        Array.isArray(window.__OneBeigeSteamSettings.schema)
      ) {
        return Promise.resolve(window.__OneBeigeSteamSettings);
      }
    } catch (_) {}
    ...
    if (
      ...
      return Promise.reject(new Error(lt("OneBeigeSteam backend unavailable")));
    }

    return Millennium.callServerMethod("onebeegsteam", "GetSettingsConfig", {
      contentScriptQuery: "",
    }).then(function (res) {
      const payload = typeof res === "string" ? JSON.parse(res) : res;
      ...
      window.__OneBeigeSteamSettings = config;
      return config;
    });
  }
  ...
  function closeSettingsOverlay() {
    try {
      ...
      var list = document.getElementsByClassName("onebeegsteam-settings-overlay");
      ...
      var list2 = document.getElementsByClassName("onebeegsteam-overlay");
  ...
  function showOneBeigeSteamAlert(title, message, onClose) {
    if (document.querySelector(".onebeegsteam-alert-overlay")) return;

    ensureOneBeigeSteamStyles();
    ensureFontAwesome();
    const overlay = document.createElement("div");
    overlay.className = "onebeegsteam-alert-overlay";
    overlay.style.cssText =
    ...
    titleEl.textContent = String(title || "OneBeigeSteam");
    ...
    okBtn.href = "#";
    okBtn.className = "onebeegsteam-btn primary";
    okBtn.style.cssText =
  ...
  function ShowOneBeigeSteamAlert(title, message) {
    try {
      showOneBeigeSteamAlert(title, message);
    } catch (err) {
      backendLog("OneBeigeSteam: Alert error, falling back: " + err);
      try {
        alert(String(title) + "\n\n" + String(message));
      } catch (_) {}
    }
  ...
  function showOneBeigeSteamConfirm(title, message, onConfirm, onCancel) {
    // Always close settings popup first so the confirm is visible on top
    closeSettingsOverlay();

    // Create custom modern confirmation dialog
    if (document.querySelector(".onebeegsteam-confirm-overlay")) return;

    ensureOneBeigeSteamStyles();
    ensureFontAwesome();
    const overlay = document.createElement("div");
    overlay.className = "onebeegsteam-confirm-overlay";
    overlay.style.cssText =
      "position:fixed;inset:0;background:rgba(0,0,0,0.8);backdrop-filter:blur(12px);z-index:100001;display:flex;align-items:center;justify-content:center;";
    ...
    cancelBtn.href = "#";
    cancelBtn.className = "onebeegsteam-btn";
    ...
    confirmBtn.href = "#";
    confirmBtn.className = "onebeegsteam-btn primary";
  ...
  function ensureStyles() {
    if (!document.getElementById("onebeegsteam-spacing-styles")) {
      const style = document.createElement("style");
      style.id = "onebeegsteam-spacing-styles";
      style.textContent = `
                .onebeegsteam-restart-button { margin-left: 0 !important; margin-right: 3px !important; }
                .onebeegsteam-button { margin-left: 0 !important; margin-right: 0 !important; position: relative !important; }
                .onebeegsteam-pills-container {
                ...
                .onebeegsteam-pill.red { background: rgba(255, 80, 80, 0.15); color: #ff5050; border: 1px solid rgba(255, 80, 80, 0.3); }
                .onebeegsteam-pill.green { background: rgba(92, 184, 92, 0.15); color: #5cb85c; border: 1px solid rgba(92, 184, 92, 0.3); }
                .onebeegsteam-pill.yellow { background: rgba(255, 193, 7, 0.15); color: #ffc107; border: 1px solid rgba(255, 193, 7, 0.3); }
                .onebeegsteam-pill.orange { background: rgba(255, 136, 0, 0.15); color: #ff8800; border: 1px solid rgba(255, 136, 0, 0.3); }
            `;
  ...
  function updateButtonTranslations() {
    try {
      ...
      const restartBtn = document.querySelector(".onebeegsteam-restart-button");
      if (restartBtn) {
      ...
      // Update Add via OneBeigeSteam button
      const onebeegsteamBtn = document.querySelector(".onebeegsteam-button");
      if (onebeegsteamBtn) {
        const label = lt("Add via OneBeigeSteam");
        onebeegsteamBtn.title = label;
        onebeegsteamBtn.setAttribute("data-tooltip-text", label);
        const span = onebeegsteamBtn.querySelector("span");
        if (span) {
          span.textContent = label;
        }
      }
    } catch (err) {
      backendLog("OneBeigeSteam: updateButtonTranslations error: " + err);
    }
  }

  // Function to add the OneBeigeSteam button
  ...
  function addOneBeigeSteamButton(force) {
    ...
    // swallowed, or the "Add via OneBeigeSteam" button intermittently never gets inserted.
    const now = Date.now();
    ...
    if (window.__OneBeigeSteamLastUrl !== currentUrl) {
      // Page changed - reset button insertion flag and update translations
      window.__OneBeigeSteamLastUrl = currentUrl;
      window.__OneBeigeSteamButtonInserted = false;
      window.__OneBeigeSteamGameAdded = false;
      window.__OneBeigeSteamRestartInserted = false;
      window.__OneBeigeSteamIconInserted = false;
      window.__OneBeigeSteamHeaderInserted = false;
      window.__OneBeigeSteamPresenceCheckInFlight = false;
      window.__OneBeigeSteamPresenceCheckAppId = undefined;
    ...
    if (
      headerContainer &&
      !document.querySelector(".onebeegsteam-header-button") &&
      !window.__OneBeigeSteamHeaderInserted
    ) {
      ensureOneBeigeSteamStyles();
      const headerBtn = document.createElement("button");
      headerBtn.type = "button";
      headerBtn.className = "onebeegsteam-header-button Focusable";
      headerBtn.tabIndex = "0";
      headerBtn.title = "OneBeigeSteam Settings";
      headerBtn.setAttribute("data-tooltip-text", "OneBeigeSteam Settings");
      ...
      window.__OneBeigeSteamHeaderInserted = true;
    ...
    if (targetContainer) {
      ...
      const ltPinButtons = function (c) {
        try {
          const rb = c.querySelector(".onebeegsteam-restart-button");
          const ab = c.querySelector(".onebeegsteam-button");
          if (rb && ab) {
      ...
      if (window.__OneBeigeSteamOrderObservedEl !== steamdbContainer) {
        try {
          if (window.__OneBeigeSteamOrderObserver) window.__OneBeigeSteamOrderObserver.disconnect();
          const obs = new MutationObserver(function () { ltPinButtons(steamdbContainer); });
          obs.observe(steamdbContainer, { childList: true });
          window.__OneBeigeSteamOrderObserver = obs;
          window.__OneBeigeSteamOrderObservedEl = steamdbContainer;
        } catch (_) {}
      }

      // Insert a Restart Steam button between Community Hub and our OneBeigeSteam button
      try {
        if (
          !document.querySelector(".onebeegsteam-restart-button") &&
          !window.__OneBeigeSteamRestartInserted
        ) {
          ensureStyles();
          ...
          if (referenceBtn && referenceBtn.className) {
            restartBtn.className =
              referenceBtn.className + " onebeegsteam-restart-button";
          } else {
            restartBtn.className =
              "btnv6_blue_hoverfade btn_medium onebeegsteam-restart-button";
          }
          restartBtn.href = "#";
          const restartText = lt("Restart Steam");
          restartBtn.title = restartText;
          ...
          window.__OneBeigeSteamRestartInserted = true;
      ...
      const existingBtn = document.querySelector(".onebeegsteam-button");
      if (existingBtn) {
        ensureTranslationsLoaded(false).then(function () {
          updateButtonTranslations();
        });
      ...
      if (!existingBtn && !window.__OneBeigeSteamButtonInserted) {
        // Create the OneBeigeSteam button modeled after existing SteamDB/PCGW buttons
        // In Big Picture mode, use queue button as reference; otherwise use first link in container
        ...
        // Use same custom button for both modes
        const onebeegsteamButton = document.createElement("a");
        onebeegsteamButton.href = "#";
        // Copy classes from an existing button to match look-and-feel, but set our own label
        if (referenceBtn && referenceBtn.className) {
          onebeegsteamButton.className =
            referenceBtn.className + " onebeegsteam-button";
        } else {
          onebeegsteamButton.className =
            "btnv6_blue_hoverfade btn_medium onebeegsteam-button";
        }
        const span = document.createElement("span");
        const addViaText = lt("Add via OneBeigeSteam");
        span.textContent = addViaText;
        onebeegsteamButton.appendChild(span);
        // Tooltip/title
        onebeegsteamButton.title = addViaText;
        onebeegsteamButton.setAttribute("data-tooltip-text", addViaText);

        // Normalize margins to match native buttons
        try {
          if (referenceBtn) {
            const cs = window.getComputedStyle(referenceBtn);
            onebeegsteamButton.style.marginLeft = cs.marginLeft;
            onebeegsteamButton.style.marginRight = cs.marginRight;
          }
        } catch (_) {}

        // Local click handler suppressed; delegated handler manages actions
        onebeegsteamButton.addEventListener("click", function (e) {
          e.preventDefault();
          backendLog(
            "OneBeigeSteam button clicked (delegated handler will process)",
          );
        });

        // Before inserting, ask backend if OneBeigeSteam already exists for this appid
        try {
          const match =
            window.location.href.match(
              /https:\/\/store\.steampowered\.com\/app\/(\d+)/,
          ...
          if (
            ...
            // If HasOneBeigeSteamForApp never settles (a slow/hung backend round-trip has been
            ...
            if (
              window.__OneBeigeSteamPresenceCheckInFlight &&
              window.__OneBeigeSteamPresenceCheckAppId === appid &&
              Date.now() - (window.__OneBeigeSteamPresenceCheckAt || 0) < 8000
            ) {
              return;
            }
            window.__OneBeigeSteamPresenceCheckInFlight = true;
            window.__OneBeigeSteamPresenceCheckAt = Date.now();
            window.__OneBeigeSteamPresenceCheckAppId = appid;
            window.__OneBeigeSteamCurrentAppId = appid;
            window.Millennium.callServerMethod("onebeegsteam", "HasOneBeigeSteamForApp", { appid })
              ...
              try {
                ...
                // lives in the OneBeigeSteam menu). Only show "Add via OneBeigeSteam" when the
                // game is NOT yet added.
                if (payload && payload.success && payload.exists === true) {
                  backendLog(
                    "OneBeigeSteam already present for this app; not inserting button",
                  );
                  // Resolved: game is already added, so the Add button is intentionally
                  // omitted. Mark it so ensureOneBeigeSteamUI stops retrying (button will never
                  // appear, and that's correct) instead of spinning to the attempt cap.
                  window.__OneBeigeSteamGameAdded = true;
                  window.__OneBeigeSteamPresenceCheckInFlight = false;
                  return;
                }
                if (
                  !document.querySelector(".onebeegsteam-button") &&
                  !window.__OneBeigeSteamButtonInserted
                ) {
                  // Insert after restart button (order: Restart → Add)
                  const restartExisting = steamdbContainer.querySelector(
                    ".onebeegsteam-restart-button",
                  );
                  if (restartExisting && restartExisting.after) {
                    restartExisting.after(onebeegsteamButton);
                  } else if (referenceBtn && referenceBtn.after) {
                    referenceBtn.after(onebeegsteamButton);
                  } else {
                    steamdbContainer.appendChild(onebeegsteamButton);
                  }
                  window.__OneBeigeSteamButtonInserted = true;
                  backendLog("OneBeigeSteam button inserted");
                }
                window.__OneBeigeSteamPresenceCheckInFlight = false;
              } catch (_) {
                if (
      ...
      try {
        ...
        if (!isNaN(appid)) {
          const pillBtn = steamdbContainer.querySelector(".onebeegsteam-button");
          if (pillBtn) {
            ...
            if (
              ...
              fetchGamesDatabase().then(function (db) {
                const btn = steamdbContainer.querySelector(".onebeegsteam-button");
                if (!btn) return;
                ...
                if (!pillsContainer) {
                  ...
                  pillsContainer.className = "onebeegsteam-pills-container";
                ...
                fetchFixes(appid).then(function (fixesData) {
                  ...
                  if (status !== "untested") {
                    ...
                    pill.className = "onebeegsteam-pill";
                    if (status === "playable") {
                  ...
                  if (showDenuvoPill) {
                    const pill = document.createElement("span");
                    pill.className = "onebeegsteam-pill orange";
      ...
      if (!logState.missingOnce) {
        backendLog("OneBeigeSteam: steamdbContainer not found on this page");
        logState.missingOnce = true;
      }
    }
  }

  // Retry addOneBeigeSteamButton() until its DOM containers exist, instead of the previous single silent
  ...
  // actually runs addOneBeigeSteamButton's body instead of being silently throttled away.
  //
  // window.__OneBeigeSteamReady only gets set once this settles (header button confirmed present, or retries
  ...
  function ensureOneBeigeSteamUI(attempt) {
    attempt = attempt || 0;
    try {
      addOneBeigeSteamButton(true);
    } catch (err) {
      backendLog("OneBeigeSteam: ensureOneBeigeSteamUI attempt " + attempt + " threw: " + err);
    }
    ...
    var headerReady = !!document.querySelector(".onebeegsteam-header-button");
    if (headerReady || attempt >= 12) {
      window.__OneBeigeSteamReady = true;
      return;
    }
    setTimeout(function () {
      ensureOneBeigeSteamUI(attempt + 1);
    }, 600);
  }
  ...
  function onFrontendReady() {
    ...
    setTimeout(function () {
      if (
        ...
        backendLog("[OneBeigeSteam] Gamepad detected - Navigation enabled");

        // Only show visual hint in Big Picture mode
        if (window.__LUATOOLS_IS_BIG_PICTURE__) {
          const hint = document.createElement("div");
          hint.id = "onebeegsteam-gamepad-hint";
          ...
          if (!document.querySelector("#onebeegsteam-gamepad-hint-styles")) {
            const style = document.createElement("style");
            style.id = "onebeegsteam-gamepad-hint-styles";
            style.textContent =
              "\
                            @keyframes fadeInOut {\
    ...
    try {
      if (
        ...
        Millennium.callServerMethod("onebeegsteam", "GetInitApisMessage", {
          contentScriptQuery: "",
        }).then(function (res) {
          try {
            const payload = typeof res === "string" ? JSON.parse(res) : res;
        ...
        try {
          if (window.location.hostname === "store.steampowered.com") {
            if (!sessionStorage.getItem("OneBeigeSteamLoadedAppsGate")) {
              sessionStorage.setItem("OneBeigeSteamLoadedAppsGate", "1");
              Millennium.callServerMethod("onebeegsteam", "ReadLoadedApps", {
                contentScriptQuery: "",
              }).then(function (res) {
                try {
                  const payload =
  ...
  document.addEventListener(
    ...
    function (evt) {
      ...
      const anchor = evt.target.closest(".onebeegsteam-button");
      if (anchor) {
        evt.preventDefault();
        evt.stopPropagation(); // Stop propagation to avoid conflicts
        backendLog("OneBeigeSteam delegated click");
        try {
          const match =
            window.location.href.match(
              /https:\/\/store\.steampowered\.com\/app\/(\d+)/,
          ...
          if (
            ...
            if (runState.inProgress && runState.appid === appid) {
              backendLog(
                "OneBeigeSteam: operation already in progress for this appid",
              );
              return;
            }

            // "Add via OneBeigeSteam": reflect the app's real add pipeline in the popup
            // (dynamic sources, Hubcap/key-gating, usage, FastFetch, progress),
            // headless. Removal is handled from the OneBeigeSteam menu, not this button.
            startOneBeigeSteamAdd(appid, anchor);
            return;
          }
        } catch (_) {}
      }
  ...
  function bootSettings() {
    ...
    loadThemes().then(function() {
        ...
        if (typeof ensureOneBeigeSteamStyles === "function") ensureOneBeigeSteamStyles();
    }).catch(function(e) {
        try { backendLog("OneBeigeSteam: Boot sequence failed: " + String(e)); } catch(_) {}
    });
  }
  bootSettings();
  ...
  function showLoadedAppsPopup(apps) {
    // Avoid duplicates
    if (document.querySelector(".onebeegsteam-loadedapps-overlay")) return;
    ensureFontAwesome();
    ensureOneBeigeSteamStyles();
    const overlay = document.createElement("div");
    overlay.className = "onebeegsteam-loadedapps-overlay";
    overlay.style.cssText =
    ...
    title.textContent = lt("OneBeigeSteam · Added Games");
    const body = document.createElement("div");
    ...
    dismissBtn.className = "onebeegsteam-btn";
    dismissBtn.innerHTML = "<span>" + lt("Dismiss") + "</span>";
    dismissBtn.href = "#";
    dismissBtn.onclick = function (e) {
      e.preventDefault();
      try {
        Millennium.callServerMethod("onebeegsteam", "DismissLoadedApps", {
          contentScriptQuery: "",
        });
      } catch (_) {}
      try {
        sessionStorage.setItem("OneBeigeSteamLoadedAppsShown", "1");
      } catch (_) {}
      overlay.remove();
    };
  ...
  // window.__OneBeigeSteamReady (the completion marker CefInjectorService's polling loop checks — see
  // ensureOneBeigeSteamUI() above) is set there once the button/icon setup actually settles, not here
