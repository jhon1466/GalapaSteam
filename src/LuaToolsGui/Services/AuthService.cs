...
public class AuthService
{
    ...
    private static string ResultPage(bool ok, string? error) => $$"""
        <!doctype html>
        <html><head><meta charset="utf-8"><title>GalapaSteam</title>
        <style>
        ...
        <body><div class="card">
          ...
          <p>{{(ok ? "You can close this tab and return to GalapaSteam." : WebUtility.HtmlEncode(error ?? "Please try again from the app."))}}</p>
        </div></body></html>
        """;
}
