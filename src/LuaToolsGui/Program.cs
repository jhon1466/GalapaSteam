...
public static class Program
{
    ...
    /// at %LOCALAPPDATA%\GalapaSteam\current\GalapaSteam.exe, we copy the
    /// current exe under the old LuaTools name so the DLL can find it.
    /// </summary>
    private static void DeployLauncherStub()
    {
        try
        {
            ...
            // The exe is now GalapaSteam.exe after the rename; the winmm.dll proxy
            // still hardcodes %LOCALAPPDATA%\LuaTools\current\LuaTools.exe.
            string sourceExe = Path.Combine(AppContext.BaseDirectory, "GalapaSteam.exe");
            if (!File.Exists(sourceExe)) return;

            Directory.CreateDirectory(targetDir);
            File.Copy(sourceExe, targetExe, overwrite: true);

            string sourceDll = Path.Combine(AppContext.BaseDirectory, "GalapaSteam.dll");
